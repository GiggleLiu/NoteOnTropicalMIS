...
In the following, we show a minimum example of mapping the independent problem of a 2 vertex graph (left) to a tensor network (right).
...
$\{\texttt{LEAF}, \texttt{ZERO}, \texttt{SUM}, \texttt{PROD}\}$, $data$ is a bit string as the content in a \texttt{LEAF} node,
...
\section{Additional contraction overhead using the standard tensor network notations}\label{sec:tensorbad}
...
Let $x_{\langle i,j\rangle} = x$, we have a graph polynomial,
...
Let $x_{\langle i,j\rangle} = x$, we have a graph polynomial similar to previous ones,
...
Then we map a clause to a tensor. For example, a clause $\neg a \vee b \vee \neg c$ can be mapped to a tensor labeled by $(s_a, s_b, s_c)$.
\begin{equation}
C = \left(\begin{matrix}
\left(\begin{matrix}
x & x_{b} \\
