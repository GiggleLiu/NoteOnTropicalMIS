...
In this paper, we focus on the less studied \textit{property} computation which can be harder but is crutial towards understanding a problem.
...
Given a good contraction order, contracting a tensor network in general has time complexity $2^{{\rm tw}(G)}$~\cite{Markov2008}, where ${\rm tw}(G)$ is the treewidth of graph $G$.
...
for people with physics background, it is suggested to use an alternative approach described in \App{sec:energymodel} to understand this construction and the independence polynomial from the partition function perspective.
...
In the special case of $x_i = x$ and $w_i = 1$, the tensor network contraction result directly corresponds to the independence polynomial.
