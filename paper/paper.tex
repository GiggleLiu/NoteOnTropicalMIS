...
\begin{abstract}
    ...
    the independence polynomial and the optimal configuration by generic programming tensor networks.
\end{abstract}

\section{Three approaches to compute independence polynomial}
...
\end{equation}
the contraction of this einsum network gives the independence polynomial %, we denote this contraction as \texttt{a,b,c,d,ab,ad,bc,bd,cd,ce->}.
\begin{equation}
...
To evaluate this polynomial, one can use the symbolic computing directly, that is, storing a polynomial of $x$ as a vector of factors.
...
\begin{align}
    ...
    \end{split}
\end{align}
The variable $x$ now is represented as a vector $\{0, 1\}$.
Then we insert them to the original tensor network contraction algorithm, the final result is the exact representation of the independence polynomial.
In the program, the multiplication can be evaluated efficiently with the convolution theorem.
However, this method suffers from a space overhead that propotional to the maximum independant set size because each polynomial requires a vector of such size to store.
...
However, the result is very unreliable in practise due to the non-negligible round off error of floating point operations.
...
This algebra is consistent with the one we derived in ~\cite{Liu2021} that uses the tropical tensor network solving spin glass ground states.
Here if we are only interested in obtaining $\alpha(G)$ and omit the factors, the algebra of $x$ corresponds to the max-plus tropical algebra~\cite{Maclagan2015,Moore2011}.
...
If one tries to implement the above algebra for enumerating configurations, he will find the space overhead is not propotional to the degeneracy of the optimal configuration.
It stores more than nessesary intermedite configurations. To speed up calculation, we use the result of much faster tropical number for bounding.
We first compute the optimal maximum independent set size $\alpha(G)$ with tropical numbers and cache all intermediate tensors in the contraction route.
Then we use boolean masks to mark relevent items for each cached tensor, i.e. using a boolean true to mark a tensor element with nonzero gradients and boolean false otherwise.
...
To compute the masks, we ``back propagate'' the masks through contraction route.
Consider a tropical matrix multiplication $C = A B$, we have the following inequality
\begin{align}
    A_{ij} \odot B_{jk} &\leq C_{ik}
\end{align}

Using the tropical algebra, we have
\begin{align}
    A_{ij} &\leq (\max_{k} (C_{ik}^{-1} \odot B_{jk}))^{-1}
\end{align}
where the tropical multiplicative inverse is defined as the additive inverse of the regular algebra. The equality holds if and only if $A_{ij}$ can contribute to $C$ (i.e. has nonzero gradient).
Let the mask for $C$ being $\overline C$, the backward rule for the mask reads
\begin{align}
...
This backward rule utilizes the properties that tropical algebra has multiplicative inverse for a nonzero element, while the ordering is well defined too.
This rule defined on matrix multiplication can be easily generalized to the einsum by replacing the matrix multiplication between $C^{\circ-1} \circ \overline C$ and $B^T$ by an einsum.
...
\section{Indenpendence polynomials}
\subsection{Common}
\begin{lstlisting}
using Polynomials
...
ninput(ne::NestedEinsum) = ninput(Iterators.flatten(ne))
