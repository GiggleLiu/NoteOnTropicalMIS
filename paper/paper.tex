...
This method is versertile such that by engineering tensor element algebra, the type of graph problems that can be computed using this method includes: independence number, the number of independent sets, independence polynomial, maximal independence polynomial, and both counting and enumeration of all maximum independence sets, the large-size independent sets, and all maximal independent sets.
...
One can easily find a good contraction order for the tensor network representation of a graph $G$ with thousands of vertices. This contraction order gives tensor network contraction complexity $2^{{\rm tw}(G)}$, with ${\rm tw} (G)$ the tree width of the problem graph.
To demonstrate the versatility of this tool, we apply it to a few examples including the calculations of the hard-square entropy constant, Euler characteristics of the independence complex, partition functions, and finite-temperature phase transitions on square-lattice graphs\blue{remove some examples}.
\end{abstract}
...
Although finding MIS is hard, people keep chanllenging themselves to find better algorithms for solving the MIS problem;
...
\blue{I am not sure about this complexity, this is based on the naive analysis of theorem 2.2 in ~\cite{Ferrin2014}}. There are some interesting works on efficiently approximating the independence polynomial~\cite{Harvey2018}, but in this work, we focus on computing the independence polynomial exactly.
...
\blue{introduce contraction orders}
...
\end{align*}
The requirement of being commutative is for the tensor contraction result to be independent of the contraction order. In the following sections, we show how to compute the independence polynomial,
