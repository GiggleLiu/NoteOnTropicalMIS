...
\end{corollary}
Again, the theorem can be proved by relating it with its set algebra version in \Cref{thm:complexsetk}. Although likely, one cannot collect all configurations represented by a sum-product expression tree into a vector due to its extremely large configuration space,
...
Since the depth of the expression tree is $O(|V|)$ and the cost of each arithmetic operation is also $O(|V|)$, the overall time complexity to generate a sample is $O(|V|^2)$.
Proving the theorem.
\end{proof}
