...
where 0 stands for variable $x$ having value \texttt{false} while 1 stands for having value \texttt{true}.
Then we map a clause to a tensor. For example, a clause $\neg x \vee y \vee \neg z$ can be mapped to a tensor labeled by $(s_x, s_y, s_z)$.
\begin{equation}
C = \left(\begin{matrix}
\left(\begin{matrix}
x & x \\
