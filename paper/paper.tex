...
Let us denote the neighborhood of a vertex $v$ as $N(v)$ and $N[v] = N(v)\cup \{v\}$.
...
where a disjunction of boolean literals is also called a clause.
To reduce a CNF solving to a tensor network contraction, we first map a boolean literal $a$ and its negation $\neg a$ to a same degree of freedom (label) $s_a \in \{0, 1\}$,
