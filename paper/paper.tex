...
\begin{definition}[Tensor Network~\cite{Cirac2021, Orus2014}]
    A tensor network is a quadruple of $\mathcal{N} = (\Tensors, \Sigma, O \in \Tensors, \Inputs : \mathcal{T} \rightarrow \Sigma^k)$. $\Tensors$ is a set of tensors and $O$ is the output tensor. $\Sigma$ is a the alphabet of labels. $\Inputs$ is a map from a tensor to a vector of labels with the vector length the same as the tensor rank.
\end{definition}
...
\begin{equation}
\mathcal{N}_{\rm matmul} = (\{A, B, C\}, C, T \mapsto \begin{cases} (i,j), T = A\\(j, k), T=B\\ (i, k), T=C\end{cases}, \{i,j,k\}),
\end{equation}
...
\begin{theorem}\label{thm:complexreal}
    ...
    the optimal time complexity for contracting the tensor network in \Cref{eq:mistensornetwork} for the independent set problem on graph $G = (V,E)$ is $O(|E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{thm:complexpoly}
    The independence polynomial~\cite{Harvey2018,Ferrin2014} of a graph $G = (V, E)$ can be computed in time $O(|V|\log(|V|)|E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
Since $k$ is upper bounded by the maximum independent set size $\alpha(G) \leq |V|$, the time complexity of element-wise addition and multiplication operation is upper bounded by $|V|\log(|V|)$. Combining with \Cref{thm:complexreal}, the a overall time complexity is $O(|V|\log(|V|)|E|)2^{O({\rm tw}(G))}$.
\end{proof}
...
\begin{theorem}\label{thm:complextropical}
    Let $G = (V, E)$ be a graph, its maximum independent set size $\alpha(G)$ can be computed in time $O(|E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
Since the element-wise addition and multiplication can be computed in $O(1)$ time, the complexity of contracting this tensor network is $O(|E|)2^{O({\rm tw}(G))}$ by \Cref{thm:complexreal}.
\end{proof}
...
\begin{lemma}
    The number of the largest $k$ independent sets of a graph $G = (V, E)$ can be computed in time $O(K\log K|E|)2^{O({\rm tw}(G))}$.
\end{lemma}
...
\begin{theorem}\label{thm:complexset}
    ...
    $I$ can be computed in time $O(|I||V||E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{thm:complexmis}
    ...
    $I_\alpha$ can be computed in time $O(|\mathcal{I}_\alpha||V||E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{thm:complexsetk}
    ...
    The independent sets with size $k > \alpha(G)-K$ of $G$ can be computed in time $O(|E||V|\sum_{k=\alpha(G)-K+1}^{\alpha(G)}|\mathcal{I}_k|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
Combining with \Cref{thm:complexreal}, the overall time complexity of contracting this tensor network is $O(|E||V|\sum_{k=\alpha(G)-K+1}^{\alpha(G)}|\mathcal{I}_k|)2^{O({\rm tw}(G))}$.
\end{proof}
...
\begin{theorem}
    Let $G = (V, E)$ be a graph, one of its maximum independent set can be computed in time $O(|E||V|)2^{O({\rm tw}(G))}$.
\end{theorem}
...
\begin{theorem}
    Let $G = (V, E)$ be a graph, one of its maximum independent set can be computed in time $O(|E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{thm:complexexpr}
        The independent sets $I$ of a graph $G=(V,E)$ can be enumerated in time $O(|E|)2^{O({\rm tw}(G))} + O(|V||I|)$.
\end{theorem}
\begin{proof}
...
\begin{equation}
    ...
    \end{matrix}\right).
\end{equation}
The contraction of such tensor network corresponds to a sum-product expression tree of the set of independent sets, which costs time $O(|E|)2^{O({\rm tw}(G))}$ by \Cref{thm:complexreal} since the computing time of expression tree addition and multiplication are $O(1)$.
...
\begin{lemma}\label{thm:complexexpr}
        Let $G = (V, E)$ be a graph and $\mathcal{I}_k$ be the number of independent sets with size $k$. The independent sets with size $k > \alpha(G)-K$ of can be computed in time $O(K\log K|E|)2^{O({\rm tw}(G))} + O(|V|\sum_{k=\alpha(G)-K+1}^{\alpha(G)}|\mathcal{I}_k|)$.
\end{lemma}
...
\begin{theorem}
    Let $G = (V, E)$ be a graph, its independent sets with size $k > \alpha(G)-K$ of can be unbiasedly sampled in time $O(|E|)2^{O({\rm tw}(G))} + O(|V|^2 M)$, where $M$ is the number of samples.
\end{theorem}
\begin{proof}
Similar to the proof of \Cref{thm:complexexpr}, we first obtain a sum-product tree representation of the configurations in time $O(|E|)2^{O({\rm tw}(G))}$.
...
\begin{lemma}
    Let $G = (V, E)$ be a graph, its independent sets with size $k > \alpha(G)-K$ of can be unbiasedly sampled in time $O(K\log K|E|)2^{O({\rm tw}(G))} + O(|V|^2 M)$, where $M$ is the number of samples.
\end{lemma}
...
\begin{theorem}
    Let $G = (V, E, W)$ be a weighted graph, its $K$ largest independent sets sizes can be computed in time $O(K\log K|E|)2^{O({\rm tw}(G))}$.
\end{theorem}
\begin{proof}
...
The computation of $s \odot t$ is a maximum sum combination problem that can be done in time $O(k\log(k))$ using the algorithm in \Cref{sec:maxsum}. Hence the overall time complexity of contracting the tensor network is $O(K\log K|E|)2^{O({\rm tw}(G))}$.
\end{proof}

\begin{lemma}
    Let $G = (V, E, W)$ be a weighted graph, its $K$ largest independent sets can be computed in time $O(K\log K|E||V|)2^{O({\rm tw}(G))}$.
\end{lemma}
...
To characterize the maximal independence restriction, we defined a tensor for each $v \in V$ and its neighbor $N(v)$ as
\begin{equation}\label{eq:maximal}
    ...
    \end{cases}
\end{equation}
where $N(v, k)$ is the $k$th vertex in $N(v)$ and $d(v) = |N(v)|$ is the degree of vertex $v$.
...
To characterize the matching constraint, we define a tensor for each $N(v)$ as
\begin{equation}
...
\end{cases}
\end{equation}
where $N(v, k)$ is the $k$th vertex in $N(v)$ and $d(v) = |N(v)|$ is the degree of vertex $v$, a label $\langle v, u \rangle$ is equivalent to $\langle u,v\rangle$.
$W$ tensor specifies the constraint that a vertex cannot be shared by two edges in the edge set, and an edge tensor carries the weights. The tensor network can be defined as
...
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
where $r_v$, $g_v$ and $b_v$ are colors for labeling the configurations. $B$ tensors are for specifying the coloring constraints and $W$ tensors are for labeling the solutions. The tensor network can be defined as
...
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
where variables $x_u^{w_{uv}}$ and $x_v^{w_{uv}}$ are for a cut on this edge or a domain wall in a spin glass problem. The tensor network can be defined as
...
\end{cases}
\end{equation}
where $w_v$ is the weight associated with the vertex $v$, $N(v, k)$ is the $k$th vertex in $N(v)$ and $d(v) = |N(v)|$ is the degree of vertex $v$.
...
Otherwise, if $v$ is in $D$, this tensor contributes a multiplicative factor $x_v^{w_v}$ to the output. The tensor network can be defined as
...
The standard tensor network can be used to study the counting version of the satisfiability problem~\cite{Biamonte2015}, while in the following, we will show a generic reduction from the problem of solving a CNF to a tensor network contraction for solving more solution space properties. We first map a Boolean literal $a$ and its negation $\neg a$ to the same degree of freedom $s_a \in \{0, 1\}$.
...
Suppose one has a finite set $S$ and a list of subsets of $S$, denoted as $\mathcal{V}$. Then, the set packing problem asks if some $k$ subsets in $\mathcal{V}$ are pairwise disjoint.
...
The generic tensor network for the set packing problem has a similar form as that for the independent set problem, where the vertex tensor is same as \Cref{eq:vertextensor}, while the edge tensor generalizes \Cref{eq:edgetensor} to higher dimensions such that for each $\sigma \in S$, we have the constraints on set $N(\sigma) = \{s| s\in \mathcal{V} \wedge \sigma \in s\}$
\begin{equation}
    B^{(\sigma)}_{N(\sigma, 1), N(\sigma, 2),\ldots, N(\sigma, d(\sigma))} = \begin{cases}
        1, & s_{N(\sigma, 1)}+ s_{N(\sigma, 2)} + \ldots + s_{N(\sigma, d(\sigma))}\leq 1,\\
        0, & \text{otherwise}.
    \end{cases}
\end{equation}
where $N(\sigma, k)$ is the $k$th element in $N(\sigma)$ and $d(\sigma) = |N(\sigma)|$ is the number of elements in $N(\sigma)$. The tensor network representation for the set packing problem is

\begin{equation}\label{eq:packtensornetwork}
\begin{split}
    \mathcal{T} &= \{W^{(v)} | v\in V\} \cup \{B^{(\sigma)} | \sigma \in \mathcal{S}\} \cup \{P\},\\
    \Sigma &= \{s_v | v \in V\},\\
    O &= P,\\
    \lambda &= T \mapsto \begin{cases}
                    (s_v), & T = W^{(v)}\\
                    (s_{N(\sigma, 1)}, \ldots, s_{N(\sigma, d(\sigma))}), & T = B^{(\sigma)}\\
                    (), & T = P
                    \end{cases}
\end{split}
\end{equation}
...
In the set covering problem, two sets are given: a set $\mathcal{V}$ of elements and a set $S$ of subsets of $\mathcal{V}$. The union of all the subsets covers $\mathcal{V}$. Each set $s \in S$ is associated with an positive integer weight $w_s$.
...
For each $s_\sigma$, we define a parameterized rank-one tensor indexed by it as
\begin{equation}
W^{(\sigma)} = \left(\begin{matrix}
    ...
    \end{matrix}\right)
\end{equation}
where $x_\sigma$ is a generic typed variable associated with $\sigma$ and the positive integer power of which can be define by the repeated multiplication.
For each element $v \in \mathcal{V}$, we can define a constraint over all $s \in S$ containing this element, i.e. $N(v) = \{s | s \in S \land v\in s\}$, as
\begin{equation}
...
The tensor network representation for the set packing problem is
\begin{equation}\label{eq:covertensornetwork}
\begin{split}
    \mathcal{T} &= \{W^{(\sigma)} | \sigma \in S\} \cup \{B^{(v)} | v \in \mathcal{V}\} \cup \{P\},\\
