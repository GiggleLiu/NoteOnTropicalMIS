...
More efficient algorithms to compute the MIS size exactly includes the branching algorithm~\cite{Tarjan1977,Robson1986} and dynamic programming.
Without changing the fact of exponential scaling in computing time, the branching algorithm gives a smaller base.
For example, in ~\cite{Xiao2017}, a sophisticated branching algorithm has a time complexity $1.1996^n n ^{O(1)}$.
The dynamic programming approach~\cite{Courcelle1990,Fomin2013} works better for sparse graphs with small tree width $tw(G)$~\cite{Fomin2006}, it gives an algorithms of complexity $O(2^{tw(G)}tw(G)n)$.
...
counting independent sets of different sizes, and understanding the effect of a local gadget.
...
One can encode the independence polynomial~\cite{Levit2005, Ferrin2014, Harvey2017} of $G$ to a tensor network.
