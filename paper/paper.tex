...
In combinatorial optimization, the most studied version of the above problems is finding the maximum/minimum set size and one of the solution with that size satisfying the contraints,
while many of them belong to the hardness category NP-hard~\cite{Hastad1996}, i.e. people believe there is unlikely a polynomial time algorithm for solving them.
In this paper, we study the property computing that can be harder but also more useful for people to understand a problem.
The properties include finding the maximum/minimum size, counting sets of a give size, enumerating all such sets or direct sampling them when they are not enumeratable.
For weighted version, it also includes finding sets with the largest/smallest $k$ sizes.
%are many interesting and hard computational problems concerning various properties of independent sets.

For avoid the discussion being too diversed, we limit the target problem to the independent set problem,
...
\section{Tensor network representation of independent sets} \label{sec:indpoly}
...
\section{Enumerating and sampling independent sets}
