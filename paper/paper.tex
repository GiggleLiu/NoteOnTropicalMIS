...
Finding maximal independent sets of a graph is equivalent to finding the maximal cliques of its complement graph, so in the following we only discuss how to find maximal independent sets.
Sometimes, one may be interested in knowing maximal independent sets to understand why his or her program for solving MIS is trapped in a local minimum.
...
Then the max cut problem can be encoded to tensor networks by mapping an edge $(i,j)\in E$ to an edge matrix labeled by $(s_i, s_j)$
\begin{equation}
