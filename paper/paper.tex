...
The word ``einsum'' is a shorthand for Einstein's summation, but it is actually invented by a group of programmers.
...
or $C_i^k = A_i^j B_j^k$ in the Einstein's original notation, where the paired subscript and superscript $j$ is a dummy index summed over.
...
This is because a label in a tensor network representation always appear exactly twice.
...
\centerline{\begin{tikzpicture}
    ...
    \draw [black,thick] (\ax+\dx,\ay) -- (\ax+\dx-0.6,\ay);
    \draw [black,thick] (\bx+\dx,\by) -- (\bx+\dx+0.6,\by);
...
For example, $C_{ijk} = A_{jkm} B_{mia} V_{jm}$ is a elgal einsum representation, all indices not appearing in the output are summed over, i.e. it represents $C_{ijki} = \sum_{ma}A_{jkm} B_{mia} V_{jm}$.
Whether the index appear as a superscript or a subscript makes no sense now.
The graphical representation of an einsum is called a hypergraph, where an edge can be shared by any number of nodes.
...
\centerline{\begin{tikzpicture}
    ...
    \node[color=cyan] at (\bx+\dx+0.5,\bx-0.5) {a};
...
In the main text, we stick to the einsum notation, although to most physicists, they are more familiar with tensor networks,
and an einsum network can be translated to an equivalent tensor network by adding $\delta$ tensors (a generalization of identity matrix to higher order).
...
\end{equation}
where a tensor index $s_i$ is a boolean variable that being 1 if vertex $i$ is in the independent set, 0 otherwise. The contraction of this einsum network gives the independence polynomial %, we denote this contraction as \texttt{a,b,c,d,ab,ad,bc,bd,cd,ce->}.
\begin{equation}
