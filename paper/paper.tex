...
This method is versertile. By engineering tensor element algebra, the type of graph problems that can be computed using this method includes: independence number, the number of independent sets, independence polynomial, maximal independence polynomial, and both counting and enumeration of all maximum independence sets, the large-size independent sets, and all maximal independent sets.
...
We can easily contract a tensor network with thousands of tensors with an algorithmic complexity $2^{{\rm tw}(G)}$, with ${\rm tw} (G)$ the tree width of the problem graph.
...
\blue{independence polynomial at -1~\cite{Bousquet2008,Levit2009},
...
For an undirected graph $G = (V,E)$, an independent set $I \subseteq V$ is a set of vertices that for any $u,v \in I$, $(u, v) \not\in E$. %there is no edge connecting $u$ and $v$ in $G$.
The primary insterest of many computational complexity theorists is finding the maximum size of such vertex sets be $\alpha(G) \equiv \max_{I}|I|$ and one of the sets having this size.
...
Although it is a hard problem, people keep chanllenging themself to find a better algorithm for solving the MIS problem;
...
In this paper, we do not limit our discussion the finding one of the maximum indepedent sets and its size.
There are many other interesting problems pertaining to independent sets, such as the number of independent sets,
...
they can, for example, be used to understand phase transitions, to analyse the overlap gap properties~\cite{Gamarnik2013, Gamarnik2019} and to identify harder graphs in an ensemble of graphs \red{cite experiment}.
...
Its algorithmic complexity is similar to that of dynamic programming, i.e. scales exponentially as the tree width of the graph ${\rm tw}(G)$.
...
We benchmark our algorithms by computing various independent set properties on CPUs and GPUs.
...
while the optimized contraction order gives us a near optimal space complexity $2^{\sim{\rm tw}(G)}$ to contract the tensor network.
...
The implementation of our algorithm is publicly available online: ~\cite{repolink}.
Lastly, we provide a few examples and show that this toolbox can be used to calculate the hard-square entropy constant,
...
i.e., contraction, and it can be represented as a multigraph with open edges by viewing a tensor as a vertex,
a label pairing two tensors as an edge, and the remaining unpaired labels as open edges.

\begin{example}
   A standard tensor network $C_{il} = A_{ijkm} B_{kml} V_{j}$ has the following multigraph representation.
...
In the main text, we use a generalized tensor network notation that an index can appear arbitrary times.
It is sometimes called einsum,
sum-product network or factor graph~\cite{Bishop2006} in different contexts.
The graphical representation of a generlized tensor network is a hypergraph, where an edge (label) can be shared by an arbitrary number of vertices (tensors).
These two notations are equivalent in the representation power because one can easily translate a generalized tensor network to the standard notation by adding identity tensors denoted as $\delta$ tensors.
However, by adding the $\delta$ tensors, one may unnecessarily increase the contraction complexity of a graph, which we illustrate in \App{app:tensorbad}.

\begin{example}
$C_{ijk} = A_{jkm} B_{mil} V_{jm}$ is a tensor network representing $C_{ijk} = \sum_{ml}A_{jkm} B_{mil} V_{jm}$,
where $i,j,k$ in the output tensor are open edges (note that $l$ is not an open edge and thus summed over).
...
computing independence polynomial (panel c) and enumerating and sampling independent sets (panel d).
\blue{I cleaned up this paragraph, because one can check the table \Tbl{tbl:generictypes} now.}
...
Among the benchmarks, computing \texttt{T (CPU)},
\texttt{$\mathbbm{R}$ (CPU)}, \texttt{$\mathbbm{R}$ (GPU)}, \texttt{$\mathbbm{C}$ (CPU)}, \texttt{$\mathbbm{C}$ (GPU)} and \texttt{T+bounding (CPU)} can utilize fast BLAS functions, and hence 
...
Fourier transformation based method is the fastest in computing the independence polynomial,
while the finite field (GF$(p)$) approach is the only method that does not incur round-off errors and that can be run on a GPU.
In the benchmark of enumerating the MISs (panel d), one can see the technique to bound the enumeration space improves the performance for more than one order.
Bounding can also reduce the memory usage significantly, without which the largest enumerable size is about $150$ vertices on a 32G memory device.
...
\section{Why not using the standard tensor network notations}\label{app:tensorbad}
As we have mentioned in the main text,
a standard tensor network notation inevitably introduces $\delta$ tensors to encode some graph problems.
A $\delta$ tensor of rank $d$ is defined as
\begin{equation}
...
Its standard tensor network representation includes a $\delta$ tensors on each vertex, so its graphical representation is as bellow.
...
\end{tikzpicture}}
Here, we use a small circle on a vertex to represent a diagonal $\delta$ tensor. Its rank is $8$ in the bulk. If we contract this tensor network in a naive column-wise order, the maximum intermediate tensor has a rank $\sim3L$, requiring a storage of size $\approx 2^{3L}$.
...
The vertex tensor is always rank $1$ in this representation.
Again, if we contract this tensor network in the columnwise contraction order, we can see the maximum intermediate tensor has rank $\sim L$ by counting the colors.
