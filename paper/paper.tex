...
The algebra of the exponents becomes the max-plus tropical algebra: $x \oplus y = \max(x,y)$ and $x \odot y = x + y$~\cite{Maclagan2015, Moore2011}.
...
\begin{figure} 
    ...
    \caption{Benchmark results for computing different properties of independent sets on a random three regular graph with different tensor element types. \red{for each graph size, is there only one graph?} (a) treewidth versus the number of vertices for the benchmarked graphs. 
