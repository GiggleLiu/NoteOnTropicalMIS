...
Only algebras requiring dynamic sized structures cannot be computed on GPU, which includes \texttt{PN} and \texttt{SN}.
When computing the independence polynomial, the finite field (GF$(p)$) approach is the only method that does not incur round-off errors and that can be run on a GPU.
