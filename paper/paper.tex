...
\begin{abstract}
We introduce a method that uses generic programming tensor network to compute various properties of a class of hard graph problems that can be quantified locally.
...
The most studied version of the above problems is finding the maximum/minimum set size and one of the sets of that size satisfying the contraints,
...
In this paper, we focus on the less studied \textit{property} computation which can be harder but is crutial towards understanding a hard problem.
Here, the \textit{property} refers to a class of quantities that not only include the maximum/minimum set size,
...
Given a good contraction order, the time complexity of contracting a tensor network is $2^{{\rm tw}(G)}$~\cite{Markov2008}, where ${\rm tw}(G)$ is the treewidth of graph $G$.
...
Although this time complexity is just on par with the dynamic programming algorithms~\cite{Courcelle1990, Fomin2013} for solving the maximum size and one of the best set, tensor network has richer algebraic structure, and this structure is crutial for property computation.
...
To avoid the discussion being unnecessaryly diversed, we mainly focus on the independent set problem,
and discuss other combinatorial optimization problems in \App{sec:otherproblems}.
...
\ra{1.3}
   ...
   {Polynomial truncated to $k$-th order (\Eq{eq:countingtropical} and \Eq{eq:max2poly})}     & {$k$ largest independent sizes and their counting} \\
...
for people with physics background, it is suggested to use an alternative approach described in \App{sec:energymodel} to understand this construction from the partition function perspective.
Let $G=(V,E)$ be a weighted undirected graph with each vertex $i\in V$ being associated with a weight $w_i$.
...
In the special case of $x_i = x$ and $w_i = 1$, the tensor network contraction result directly corresponds to the independence polynomial~\cite{Harvey2018,Ferrin2014}.
...
Similarly, one can also truncate the polynomial and keep only its smallest several orders, which can be used for e.g. counting the minimum maximal independent sets.
...
The vertex tensor and edge tensor become
\begin{equation}
    W^{{\rm T}k}(-\infty^{\otimes k-1} \otimes 1) = \left(\begin{matrix}
        \mymathbb{1} \\
        -\infty^{\otimes k-1} \otimes 1
    \end{matrix}\right),   
    \qquad
...
Note here, the $\odot$ of configuration sampler is not used so that the resulting configurations are deterministic and complete.
...
\end{equation}
where $s_i$ is a spin on vertex $i \in V$ and $w_i$ is an onsite energy term.
...
\begin{equation}
    ...
    Z(G, \beta) = \sum_{s}e^{-\beta \mathcal{E}(G, s)} = \sum_{s\in \mathcal{I}(G)} e^{\beta \sum w_i s_i}\\
         = \sum_{k=0}^{\alpha(G)}D(k) e^{\beta k}  \qquad \quad (w_i = 1)
    \end{split}
\end{equation}
where $\mathcal{I}(G)$ is the set of independent sets of graph $G$.
The second line corresponds to the unweighted independent set problem,
$\alpha(G)$ is the absolute value of the minimum energy (maximum independent set size) and $D(k)$ is the number of states with energy $-k$ (independent sets of size $k$).
It can be evaluated as a tensor network contraction,
where the tensor network is constructed by placing a vertex tensor on each vertex $i$
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
and a bond tensor on each edge
\begin{equation}
       B = \left(\begin{matrix}
        1  & 1\\
        1 & 0
    \end{matrix}\right),
\end{equation}
where the $0$ in the bond tensor comes from $e^{-\beta\infty}$ from the second term of the energy model.
By letting $x = e^{\beta w_i}$, we get the tensor network for computing the independence polynomial as described by \Eq{eq:vertextensor} and \Eq{eq:edgetensor}.
...
\section{Generalization to other graph problems}\label{sec:otherproblems}
...
For a vertex $v$, we define the degrees of freedom $c_v\in\{0,1,2\}$ and a vertex tensor labelled by it as
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
where $r_v$, $g_v$ and $b_v$ are colors used to label the configurations.
...
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
where weights are associated with edges. The number of possible colorings can be obtained by contracting this tensor network by setting $r_v, g_v$ and $b_v$ to $1$.
...
where its $k$th coefficient is the number of coloring with $k$ bonds are satisfied.
...
\begin{equation}
    B(x, w_{\langle i, j\rangle}) = \left(\begin{matrix}
        1 & x_{j}^{w_{\langle i,j\rangle}}\\
        x_{i}^{w_{\langle i,j\rangle}} & 1
    \end{matrix}\right),
\end{equation}
where variables $x_i$ and $x_j$ represents a cut on edge $(i, j)$ or a domain wall of an Ising spin glass.
Let $x_i^{w_{\langle i,j\rangle}} = x_j^{w_{\langle i,j\rangle}} = x$, we have a graph polynomial similar to previous ones,
...
We can contruct a satisfiability problem to solve the above problem.
To generate a tensor network, we map a boolean variable $a$ and its negation $\neg a$ to a degree of freedom (label) $s_a \in \{0, 1\}$,
...
\end{matrix}\right),
\end{equation}
where a weight is associated with a clause. There is only one entry $(s_x, s_y, s_z) = (1, 0, 1)$ that makes this clause unsatisfied.
If we contract this tensor network, we will get a multiplicative factor $x$ whenever there is a clause satisfied.
Let $x_{ab\ldots} = x$, then we can get a polynomial that its $k$-th coefficients gives us the number of assignments with $k$ clauses satisfied.
