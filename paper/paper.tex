...
In previous works combining tensor network and combinatoric problems~\cite{},
...
The key technology is called the generic programming:
...
This definition of generic programming contains two major pieces, one is a single program for the general setting, and another is performance.
To understand the first point, suppose we want to write a function to take the power of an element $f(x, n) := x^n$,
the generic programming approach does not assume $x$ being a regular number,
...
In such a way, when the program takes a matrix as input, it computes the matrix power without rewriting this function for matrices.
The second point about performance basically rules out dynamic typed languages like Python,
because in these languages, the efficiency of a generic type is not guaranteed.
...
When the program ``sees'' a new input type, the compiler recompiles the generic program for the new type.
A lot optimization can be done during the compiling, like inlining immutable elements with fixed sizes in an array.
...
We find as long as the tensor elements form a commutative semiring, the tensor network contraction result can be contraction order independent.
...
\red{It might be helpful to add a figure of the tensor network for computing the maximal independence polynomial.} 
