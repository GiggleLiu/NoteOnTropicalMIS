...
The example above can be written as $C_{ijkl} = A_{jkm} B_{mil} V_{jm} = A_{ukm} B_{pil} V_{vq} \delta_{mpq} \delta_{juv}$ \red{update this} in the standard notion. However, by adding the $\delta$ tensors, one may unnecessarily increase the contraction complexity of a graph, which we illustrate in \App{app:tensorbad}.
...
We hope our algorithms to calculate the independence polynomial can also help further research in these directions. \red{possibly add set packing problem relationship} 
...
\end{equation} \red{remove subscript $s_{i} s_{j}$ and add description after the next equation}
...
Mapping a graph (left) to a tensor network (right) that encodes the independence polynomial. In the generalized tensor network's graphical representation, a vertex is mapped to a hyperedge, and a vertex tensor is connected to the hyperedge. An edge is mapped to an edge tensor connected to the hyperedge.
\red{come back to this to change}\blue{$\checkmark$}
...
\begin{algorithm}[!ht]
    ...
    \While{true}{
        ...
        compute the tensor contraction for each $x = \chi_i \pmod p$ on $\text{GF}(p)$ and obtain $Y = (y_0, y_1, \ldots , y_{\alpha(G)}) \pmod p $\; \red{for each $i$}
