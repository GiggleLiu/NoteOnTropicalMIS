...
\begin{definition}[Tensor Network~\cite{Cirac2021, Orus2014}]
    A tensor network is a quadruple of $\mathcal{N} = (\Tensors, \Inputs, \Output, \Sigma)$ where $\Sigma$ is a the alphabet of labels, $\Tensors = (\Tensors_1, \ldots, \Tensors_m)$ is a vector of tensors, $\Inputs$ is a map from tensor to a vector of labels $\Inputs: \Tensors_j \mapsto \mathbf{l}_j$, $(\mathbf{l}_j)_i \in \Sigma, i=1,\ldots, \rm{rank}(\Tensors_j)$.
...
\begin{theorem}
    The independence polynomial~\cite{Harvey2018,Ferrin2014} of a graph $G = (V, E)$ can be computed in time $O(|V|^22^{{\rm tw}(G)})$.
\end{theorem}
\begin{proof}
    By contracting 
\end{proof}
