...
In this paper, we focus on computing various independent set \textit{properties}. % We introduce a generic programming tensor-network framework to compute the various \textit{properties} pertaining to independent sets. 
...
%in statistical mechanics and the Rydberg hamiltonian with neutral atoms~\cite{Pichler2018, Ebadi2022} \red{replace with experiment paper when ready};
%they can, for example, be used to understand phase transitions, to identify harder graphs in an ensemble of graphs~\cite{Ebadi2022} and to analyze the overlap gap property~\cite{Gamarnik2013, Gamarnik2019}. 
We map the computation of these independent set properties into generic tensor network contraction with specially designed tensor element algebra.
...
The complexity of our tensor-network based algorithms is similar to that of dynamic programming algorithms~\cite{Courcelle1990, Fomin2013} that dedicated to solving the MIS size,
...
This indicates it is a method that works better for sparse graphs,
...
We benchmark our algorithms in \Sec{sec:benchmark} by computing various independent set properties of certain sparse graphs on central processing units (CPUs) and graphics processing units (GPUs) and show its good performacne.
The high performance of our algorithms benefits from recent advances in random tensor-network contraction for the purposes of quantum circuit simulations~\cite{Gray2021, Pan2021, Kalachev2021},
...
Figure (a) shows the time and space complexity of tensor network contraction for variety of graph sizes, where the space complexity is the same as the tree width of the problem graph.
The contraction order is obtained using the local search algorithm in Ref.~\cite{Kalachev2021}.
In practice, slicing technique is used for graphs with tree width greater than 27 to fit the computation into a 16GB memory.
