...
\blue{overlap gap property~\cite{Gamarnik2013, Gamarnik2019}, independence polynomial at -1~\cite{Bousquet2008,Levit2009}, treewidth of 3-regular graphs ~\cite{Fomin2006},
...
\red{I've changed this. please check. The original was $C_{ijk} = A_{jkm} B_{mil} V_{jm}$ and $C_{ijk} = \sum_{ml}A_{jkm} B_{mil} V_{jm}$}.
\blue{changed back because $l$ is not an open edge. Is it still confusing if I introduce open circles for open edges?}
...
\centerline{\begin{tikzpicture}[
    ...
    \node[color=white,draw=black,dot=\sr] at (\ax-1,\by) (o1) {};
    \node[color=white,draw=black,dot=\sr] at (\ax-1.5,\ay) (o2) {};
    \node[color=white,draw=black,dot=\sr] at (\ax-1,\cy) (o3) {};
...
Our finding is: as long as the tensor elements form a commutative semiring, the tensor network contraction result can be contraction order independent.
So what is a commutative semiring?
...
Mapping a graph (left) to a tensor network (right) that encodes the independence polynomial. In the generalized tensor network's graphical representation, a vertex is mapped to a hyperedge, and an edge is mapped to an edge tensor.
\red{a vertex is mapped to a hyperedge?}\blue{$\checkmark$}
...
\begin{equation}
    ...
    \mymathbb{0} &= (), \text{\red{should this be (0)?}}\blue{\text{both are ok}} \\
...
Let use denote the results as $Y$, then we solve a linear equation $Y = \hat X A_p$ using the gaussian elimination on ${\rm GF}(p)$ to find the coefficient moduluses $A_p \equiv A \pmod p$.
Finally we apply the Chinese remainder theorem to get $A \pmod P$, where $P$ is a product of all prime numbers chosen in current and previous iterations.
...
\begin{algorithm}[!ht]
    ...
    \While{true}{
        ...
        compute the tensor contraction for each $x = X_i \pmod p$ on $\text{GF}(p)$ and obtain $Y = (y_0, y_1, \ldots , y_{\alpha(G)}) \pmod p $\;
        ...
        $P = P \times p$\;
    }\caption{Compute the independence polynomial exactly without integer overflow}\label{alg:finitefield} \red{discuss this algorithm table, $X_{ij}$, etc.}\blue{done}
\end{algorithm}

\subsection{Maximal independence polynomial}
\red{It might be helpful to add a figure of the tensor network for computing the maximal independence polynomial.} \blue{done}
...
\begin{equation}
    ...
    \mymathbb{0} &= (), \text{\red{should this be (0)?}} \blue{\text{both are fine}} \\
...
\blue{bitwise OR is more often used. I think Hadamard xxx means broadcasted xxx, is this confusing?}
...
\blue{we do not need an extra definition, see my example.}
...
\begin{split}
    ...
    \mymathbb{0} &= 1^{\otimes |V|}, \red{\text{should be } \mymathbb{0} = \{\}?}\\
     &\blue{\text{I do not think so, in that way, we need an extra bit to represent an empty set}} \\
    \mymathbb{1} &= 0^{\otimes |V|},
...
\end{equation}
On the right hand side, we transform the operation into a tropical matrix multiplication so that we can utilize the fast tropical BLAS routines~\cite{TropicalGEMM} \red{do we need to cite here?}\blue{would a link to github be good?}.
...
By choosing different $r$, one can obtain better precision in low independent set size region  ($r<1$) or high independent set size region ($r>1$). \red{is that $\omega < 1$ or $r <1$?}\blue{it is $r$, fixed}
