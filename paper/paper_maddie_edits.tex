...
\begin{abstract}
We introduce a method that uses generic programming tensor network to compute various properties of a class of hard graph problems that can be quantified over a local set.
...
Other hard graph problems solvable in our framework includes the cutting problem, the K-coloring problem, the maximal clique problem, the dominating set problem, and satisfiability problem, among others.
...
We show how our method helps people understand these hard problems better.
\end{abstract}
...
\blue{I am not sure I an use the word ``quantify'' in a tensor network!
It is borrowed from the field of logic,
but I am feeling defining a local tensor in our paper is same as quantifing over a variable in logic.
\href{https://en.wikipedia.org/wiki/Quantifier\_(logic)}{https://en.wikipedia.org/wiki/Quantifier\_(logic)}}

\section{Introduction}
In graph theory and combinatorial optimization, there is an important class of problems that can be quantified over a vertex and its neighborhood,
including the independent set problem, the cutting problem, the K-coloring problem, the satisfiability problem,
the maximal clique problem, and the vertex cover problem et al ~\cite{Moore2011}. \purple{Not clear to me what ``over a vertex and its neighborhood'' means. }
...
\blue{need more diversed applications not limited to IS} \purple{Here I would say that that these problems are intractable (exponential runtime in problem size in the worst-case), but it is nevertheless desirable to have high-performance algorithms because of their ubiquitous applications.}

The most studied version of the above problems is finding a maximum/minimum set satisfying the constraints and the corresponding set size,
while many of them belong to the hardness category NP-hard~\cite{Hastad1996}, i.e. it is unlikely to solve them in a polynomial time. \purple{Is MIS really the most studied NP-hard optimization problem? I would guess it is heavily studied, but not necessarily the most studied.}
In this paper, we focus on the less studied \textit{property} computation which can be harder but is crucial towards understanding a hard problem. \purple{It might be interesting to note that computing the number of sets of a given size is \#P-hard (class of all counting problems), which is believed to be much harder than NP.}
Here, the \textit{property} \purple{Consider using ``solution space property'' instead of just ``property''.} refers to a class of quantities that not only include the maximum/minimum set size,
...
For the weighted version of the above problems, it also includes finding largest/smallest $k$ sets and their sizes. \purple{I would give examples of why we want to compute solution space properties. For instance, in statistical physics, computing thermal states and observables via the partition function. In complexity theory, proving limits on local algorithms (OGP). }
%are many interesting and hard computational problems concerning various properties of independent sets.

We propose to view the computation of all these properties from an abstract algebra perspective and use a unified framework,
generic tensor-network, to solve these seeming unrelated properties all.
...
Tensor network is also known as the sum product network or einsum in different fields, \purple{Say which fields}
where its tensor element types are typically restricted to standard number types such as real numbers and complex numbers. \purple{Briefly say what tensor networks are in a few words, so that ``restricted to standard number types such as real numbers and complex numbers'' can be put into context. A reader new to tensor networks might be wondering what a tensor network is, and the only context we get from this sentence is that they involve numbers}
...
Given a good contraction order, the space required to contract a tensor network is 2 power the tree width of its line graph~\cite{Markov2008}. \purple{I'd write out $2^{\text{tw}(G)}$.}
Recent progress in simulating quantum circuits with random tensor networks~\cite{Gray2021, Pan2021, Kalachev2021} enables us to find such a good contraction order for a tensor network with thousands of tensors in a reasonable time. \purple{Is the scaling of these algorithms known?}
When using tensor network for solving one of the best sets with the maximum size,
this time complexity is on par with the dynamic programming algorithms~\cite{Courcelle1990, Fomin2013},
however, tensor network has richer algebraic structure, and this structure is crucial for property computation. \purple{Not clear how first part and second part of sentence are related}
...
To avoid the discussion being unnecessarily complicated, we  focus concretely on the independent set problem,
and discuss other combinatorial optimization problems in \App{sec:otherproblems}. \purple{More details on which combinatorial optimization problems this applies to?}
...
In the standard Einstein's notation for tensor networks in physics, each index appears precisely twice \purple{signifying which pairs of indices are summed over}.
...
However, introducing $\delta$ tensors may significantly increase the complexity of the tensor contraction. We illustrate this subtle point in \App{sec:tensorbad}. \purple{ I feel that the $\delta$ tensors point is unnecessary to mention in the main text. Because not much detail is given, I internalized what it meant only on the third read-through.}
...
\purple{On a second read of this section, I think that the clarity could be improved. My understanding is this is the flow: 
We would like to use generic programming, where we write code that works regardless of input type, and is optimized on all input types without sacrificing efficiency. 
In general, we can write code such that a function runs correctly regardless of the input data type. This is easy to do for dynamically typed languages like Python, which only verify and enforce the rules for the input data type at runtime (called \textit{type-checking}). However, when the input data type is only checked at runtime (as opposed to when the code is compiled), type-specific optimization cannot be used. High efficiency  is only achieved in statically typed languages, where modern compiling technology allows the function to be individually optimized for each input data type (then give C++/Julia examples). }
...
Owing to the development of modern compiling technology, we no longer need to restrict our imagination to standard number types.
...
To understand the first aspect on generality, \purple{This is a really helpful example!} suppose we want to write a function that raises an element to a power, $f(x, n) := x^n$.
...
In such a way, when the program takes a matrix as an input, it computes the matrix power without extra effort.
...
This motivates us to think about what is the most general element type \purple{I would clarify element type means tensor element type} allowed in a tensor network contraction program.
...
\end{align*}
We require the algebra be multiplicative commutative because the contraction order optimizer will change the multiplication order of elements, while we wish this contraction order does not change the contraction results. \purple{The previous sentence makes a lot of sense!}
...
\ra{1.3}
   ...
   {Polynomial truncated to $k$-th order (\Eq{eq:countingtropical} and \Eq{eq:max2poly})}     & {$k$ largest independent set sizes and their counting \purple{``and their counting'' is gramatically confusing, can we say independence polynomial or degeneracy or something?}} \\
   {Set} (\Eq{eq:set})     & {Enumeration of independent sets} \\
   {Sum Product Tree} (\Eq{eq:expr})     & {Sampling of independent sets} \\
...
For people with a physics background, we suggest using an alternative approach described in \App{sec:energymodel} to understand this reduction from an energy model.
Let $G=(V,E)$ be a weighted, undirected graph with each vertex $i\in V$ being associated with a weight $w_i$ \purple{I suggest being consistent in subscripts, and use $u/v$ for nodes as you do later, e.g. $v \in V$ instead of $i/j$}
...
In the following, we show a minimum example of mapping the independent problem of a complete graphs with two vertices K2 (left) to a tensor network (right).
...
\end{example}
\purple{This example is really helpful!}
 
It is in general not computational efficient to evaluate \Eq{eq:idp} by directly summing up the $2^{|V|}$ products. \purple{Perhaps explicitly say what this scales as?}
...
A fully optimized contraction order can reduce the time to $O(2^{\text{tw}(G)})$~\cite{Markov2008}, while requiring a space at the same order of magnitude to store intermediate contraction results. \purple{Perhaps just say time and space complexity? Order of magnitude doesn't make sense to me in terms of scaling. Also, what is the time complexity of finding a good contraction order? Is there a known average-case complexity? This seems important.}
...
In the following, we mapping a 5 vertex graph (left) to a tensor network (right) and show how the contraction order simplifies the computation.
...
One can easily check the maximum tensor during computing is rank 2, and the total number of multiplication operations is reduced significantly. \purple{I have not checked this yet but will tomorrow. Is it really easy/simple enough to do without writing stuff down? Is this the simplest example possible? The equations look quite formidable at first glance, and it takes a lot of time to match the hyperedge labels in the equation to the physical graph.}
\end{example}
...
One can feed the standard computer numbers types into this tensor network and evaluate the independence polynomial by contracting it numerically. \purple{By feed in standard computer number types into the network, do you mean for $x$? Then, you look at multiple $x$, you can fit the independence polynomial to solve for the coefficients? It's not very explicit this procedure works at this point.}
However, we are more interested in finding the coefficients of this polynomial, because this quantity tells us the counting of independent sets at each size. \purple{Does the previous technique (contracting the tensor network numerically) give the coefficients of the polynomial? The use of the word ``however'' here is confusing to me because it feels like you're saying that the previous technique doesn't give the coefficients, or it's very hard to obtain the coefficients.}
...
$\mymathbb{0}$, and multiplicative identity, $\mymathbb{1}$, of a commutative semiring as discussed in Sec.~\ref{sec:generic} \purple{I'd say that you're about to describe this semiring, otherwise it seems like you're going to use the general definition of the semiring rather than a specific type of semiring}.
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Here, the multiplication operation can be evaluated efficiently using the convolution theorem~\cite{Schonhage1971}. \purple{What is the time complexity of evaluating the multiplication operation?}
...
In \App{sec:finitefield}, we provide an accurate and memory efficient method to find the polynomial by fitting it in a finite field.
Alternatively, one can use the method in \App{sec:fft} to fit this polynomial more time efficiently with complex number at the cost of precision. \purple{It's not clear to me what we gain by using the polynomial semiring rather than floating points/integers here. Could you comment on how the compiler can optimize on this data type?}
...
\end{equation}
\purple{On the first instance of ``plugging in'' infinity, it might be worth explicitly writing the limit, e.g. $I(G, \infty) =\lim_{x\rightarrow \infty}\sum_{k=0}^{\alpha(G)}a_k x^k = a_{\alpha(G)} \infty^{\alpha(G)}$}
where the lower-order terms vanish. We can thus replace the polynomial type $a = (a_0, a_1, \ldots, a_k)$ with a new type with two fields: the largest exponent $k$ and its coefficient $a_k$. \purple{I was initially confused because I thought $\mymathbb{0} = (0, ?)$ where ``?'' could be anything positive since it's multiplied by zero. It confused me that $?$ could equal $-\infty$ since I thought you defined your ring for $k$ positive, since a polynomial has positive exponents. If you have negative exponents I thought it is a Laurent polynomial or something. After re-reading I see why this doesn't matter, though.}
...
Similarly, one can also truncate the polynomial and keep only its smallest several orders, which can be used for e.g. counting the minimum maximal independent sets.\purple{Should define maximal independent sets, and minimum maximal independent sets (I would just say explicitly the smallest cardinality of a maximal independent set, which is...}
As will be shown below, this algebra can also be extended to enumerate those large-size independent sets. \purple{This section makes a lot of sense and I think this paper is really cool! The flow between sections so far has been very smooth.}
...
Directly implementing this algebra for configuration MISs might incur significant space overheads for keeping too many intermediate states not contributing to the final maximum independent sets,
...
Then instead of using the set algebra, one can use a binary sum-product tree as a compact representation of the configurations.
...
\end{equation}
\purple{It might be nice to have a picture of an example tree if you have time. However, it is pretty clear from the definition, it would just help to visualize. Do you ever use the zero element?}
The vertex tensor and edge tensor become
\begin{equation}
    W^{\rm SPT}((\texttt{LEAF},\boldsymbol{e}_i,/,/)) = \left(\begin{matrix}
        \mymathbb{1} \\
        (\texttt{LEAF},\boldsymbol{e}_i,/,/)
    \end{matrix}\right),   
    \qquad
        B^{\rm SPT} = \left(\begin{matrix}
        \mymathbb{1}  & \mymathbb{1} \\
        \mymathbb{1} & \mymathbb{0}
    \end{matrix}\right).
\end{equation}
...
one can compute the number of configurations in each branch rigorously and efficiently. \purple{To clarify - is the sum-product tree now the coefficients on the polynomial? I am a little confused on how we would, for instance, use this to compute the MISs.}
To generate samples, one just descends this sum product tree to left or right sibling with probability decided by the size of each branch while doing the computation specified by the first field. \purple{How do we compute the size of each branch?}
...
In this benchmark, we do not include traditional algorithms for finding the MIS size like the branching algorithms~\cite{Tarjan1977, Robson1986} and dynamic programming based algorithms~\cite{Courcelle1990, Fomin2013},
...
\begin{figure} 
    ...
    all independent set configurations having size $\alpha(G)$ and $\alpha(G)-1$, and all independent set configurations having size $\alpha(G)$ and $\alpha(G)-1$ in the sum product tree format,
...
In practice, slicing technique is used for graphs with treewidth greater than 27 to fit the computation into a 16GB memory. \purple{What is the slicing technique? Can you add a refernce if it's from another paper?}
One can see that all the computing times in figure~\ref{fig:benchmark}(b), (c) and (d) have a strong correlation with the treewidth \purple{with the number of vertices, and therefore the treewidth}. \purple{Does this say anything about how optimized the contraction order is? If so, it might be worth explicitly saying. Some of the quantities in panel d look like they might be superexponential in the number of vertices (tree-width).}
...
are much faster comparing to non-BLAS element types in the same category. \purple{It might be worth explicitly saying the data types the first time they're mentioned (for example, I don't know what \texttt{T} is. Also, it might be worth adding a couple descriptor words on bounding.}
...
Most algebras can be computed on a GPU, except those requiring dynamic sized structures, i.e.\ \texttt{PN}, \texttt{SN}, \texttt{T100} and \texttt{SPT} \purple{What are these things listed?}.
...
but it may suffer from round-off errors \purple{Can the round-off error thing be seen from Figure c?}. The finite field (GF$(p)$) approach is the only method that does not have round-off errors and can be run on a GPU.
...
For each property, we design an algebra being a commutative semiring and let it be the tensor element type.
...
We combine them with the bit string algebra, the set algebra, and sum product tree algebra for finding, enumeration, and sampling of independent sets at a given size.
...
A complete implementation can be found in our Github repository~\cite{GenericTensorNetworks}. 
...
\centerline{\begin{tikzpicture}[]
    ...
    \node[draw,fill=brown!75,fill opacity=1, text opacity=1.0,ellipse,minimum width=3.5cm, minimum height=1.0cm,inner sep=0pt] at (\dx, 0.7) (P1) {\hspace{-1.0cm}Highest order};
    \node[draw,fill=brown!40,fill opacity=1, text opacity=1.0,ellipse,minimum width=3.5cm, minimum height=1.0cm,inner sep=0pt] at (\dx, -0.4) (P2) {\hspace{-0.8cm}2nd highest order};
...
\end{tikzpicture}}
    ...
    \caption{The tensor network element types used in this work, while the purpose of these element types can be found in \Tbl{tbl:generictypes}.
    The overlap between two algebra indicates that a new algebra can be created by combining those two types of algebra. \purple{It's not clear what some words mean on this diagram, such as ``highest order'' and ``extended tropical''.}}
...
\begin{description}
	\item[\href{https://github.com/under-Peter/OMEinsum.jl}{OMEinsum} and \href{https://github.com/QuEraComputing/OMEinsumContractionOrders.jl}{OMEinsumContractionOrders}] are packages providing the support for Einstein's (or tensor network) notation and contraction order optimizations.
...
\href{https://github.com/QuEraComputing/GenericTensorNetworks.jl}{https://github.com/QuEraComputing/GenericTensorNetworks.jl}.
