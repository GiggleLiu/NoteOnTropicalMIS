...
{\bf Response:}
Thank you for the careful reading and constructive comments on notations. We applied the most of the suggested changes accordingly in the main text to make it more readable except the comment 3).
We can not find a better symbol to replace the existing one, so we emphasised this notation is different from the XOR in computer science when we first introduce it.
...
We re-organized the contents in the definition, theorem, proposition style to make the key points more clear to readers in the field of theoretical computational complexity and applied mathematics.
